...
const getCourses = async () => {
    ...
    try {
        let url;
        if (getSession('userType') === 'teacher') {
            url = `${apiEndpoints.GET_COURSES_ENDPOINT}/${getSession('userid')}`;
        } else {
            url = `${apiEndpoints.GET_ALL_COURSES_ENDPOINT}`;
        }
        ...
        if (data && data.data && data.data.status === 200) {
            if (getSession('userType') !== 'teacher') {
                ...
                if (purchasedData.data.status === 200) {
                    const filteredData = purchasedData.data.data.map(item => item.course._id);
                    console.log('Purchased array', filteredData);

                    const allCourses = data.data.data.map(item => ({ ...item, isPurchased: filteredData.includes(item._id) }));
                    console.log('All courses', allCourses);
